...
class DataLoader(DataLoaderBase):
    ...
    def _access_dataset(self, origin_path, groundtruth_path, datatype):
        ...
        for num in range(len(orgList)):
            loc = orgList[num].rfind(
                '\\')  # 此处可能出错，可以交换试验一下句是否可行   if this palce goes wrong,please switch to next line to have a try
            # loc=orgList[num].rfind('/')
        ...
        for index in range(len(orgList)):
            ...
            imgs[index, :, :, 0] = np.asarray(
                orgImg[:, :, 1] * 0.75 + orgImg[:, :, 0] * 0.25)  # 血管在RGB图片的G通道非常明显，在B通道最不明显
    ...
    def prepare_dataset(self):
        ...
        imgs_train, groundTruth = self._access_dataset(self.train_img_path, self.train_groundtruth_path,
                                                       self.train_type)
